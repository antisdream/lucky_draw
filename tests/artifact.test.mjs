...
const [shell,app,core,css,scenes]=await Promise.all(['shell.html','app.js','core.js','style.css','scenes.js'].map(file=>readFile(new URL(`../src/${file}`,import.meta.url),'utf8')));
...
test('네 게임은 실제로 서로 다른 캔버스 경로에 연결된다',()=>{
  ...
  for(const name of ['drawWheel','drawDart','drawMunch','drawTail'])assert.match(app+scenes,new RegExp(`function ${name}\\(`));
  assert.match(app,/drawDart\(ctx,w,h,frame,mini\)/);
});
